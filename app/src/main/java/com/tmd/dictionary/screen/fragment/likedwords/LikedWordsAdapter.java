 ...
 */
public class LikedWordsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public LikedWordsAdapter(LikedWordsContract.ViewModel viewModel) {
        mViewModel = viewModel;
        mGson = CustomGson.getGson();
    }

    public void setSource(LikedWord likedWords) {
        if (likedWords == null) {
            return;
        }
        mListKey = likedWords.getJsonWord();
        mListType = likedWords.getType();
    ...
    private class JpnViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bindData(JpnWord word) {
            mBinding.setViewModel((LikedWordsViewModel) mViewModel);
